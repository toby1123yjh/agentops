"""Authenticated self-hosted entry point reusing Dashboard APIs and permissions."""
...
from agentops.common.local_mode import get_public_url, validate_local_settings
...
from agentops.common.environment import API_URL, APP_URL
...
_allowed_hosts = {"localhost", "127.0.0.1", "[::1]", "api"}
for url in (APP_URL, API_URL):
    if hostname := urlparse(url).hostname:
        _allowed_hosts.add(hostname)

app = FastAPI(title="AgentOps self-hosted", docs_url="/docs")
...
async def local_origin_guard(request: Request, call_next):
    ...
    if origin and origin not in {APP_URL, str(request.base_url).rstrip("/")}:
        return JSONResponse({"detail": "Origin is not allowed in self-hosted mode"}, status_code=403)
...
def login(body: LoginSchema):
    ...
    response.set_cookie(AUTH_COOKIE_NAME, _encode_session_cookie(Session.create(user_id)),
                        httponly=True, secure=urlparse(APP_URL).scheme == "https",
                        samesite="strict", path="/", max_age=AUTH_COOKIE_EXPIRY)
    return response
...
@app.get("/health")
def health():
    return {"mode": "self-hosted" if get_public_url() else "local", "status": "up"}
